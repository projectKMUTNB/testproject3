...
mysql_select_db("mayz_messs") or die("Cannot select database");
...
if(isset($_GET['data_search']) && $_GET['data_search']!=""){ // ถ้า มีการส่งค่า คำค้นหามา และค่านั้นไม่ใช่ค่าว่าง
    ...
    $q_search=" AND student_code like '%".$_GET['data_search']."%' ";
}else{
...
$q="SELECT * FROM location WHERE 1 $q_search ORDER BY student_code desc limit 1 ";
$qr=mysql_query($q);

while($rs=mysql_fetch_array($qr)){
?>
<marker id="<?=$rs['student_code']?>">
 <name><?=$rs['student_code']?></name>
        <latitude><?=$rs['location_x']?></latitude>
        <longitude><?=$rs['location_y']?></longitude>
    </marker>
<?php } ?>
</markers>
