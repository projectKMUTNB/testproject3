...
//ดึงคะแนน
    ...
    $query = mysqli_query($conn,$query);
        ...
        $query1 = "SELECT DISTINCT student_code  FROM crecodestudentevaluation order by student_code asc";
...
$a=1;
    ...
    /*    $content .= '
          <td style="border-right:1px solid #000;padding:3px;text-align:left;" width="2%"  >'.array_sum($innum).'</td>';*/
...
<h2 style="text-align:center">แบบประเมินนักศึกษา</h2>
    ...
    while ($e=mysqli_fetch_array($query))
    {
      ...
      if ($e[0]==1){
        ...
        $head.= '<th style="border-right:1px solid #000;padding:4px;text-align:center;" colspan="6"  width="10%" ><center>'.$e[1] .'</center></th>';
      }if($e[0]==4){
        $head.= '<th style="border-right:1px solid #000;padding:4px;text-align:center;" colspan="6"  width="10%" ><center>'.$e[1] .'</center></th>';
      }
}
