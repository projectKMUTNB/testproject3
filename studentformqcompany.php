...
$query2 = "SELECT * FROM apprenticeship WHERE student_code = '$_SESSION[student_code]' ORDER BY student_code ASC";
...
<body>
      ...
      <div class="modal-body">
        ...
        <?  while($row = mysqli_fetch_array($result)) { ?>
            <form role="form" id="form" name="form" method="POST" action="studentformqcompanyQuery.php">
              <table class="table table-hover">
               <tbody>
                ...
                <?php
                $i=0;
                $query = "SELECT * FROM mainrecordstucompany where approve_status ='1'";
                $query = mysqli_query($conn,$query);
                $i=1;
                while ($m=mysqli_fetch_array($query))
                {
                  $a++;
                  $query1 = "SELECT crsc_name FROM choicerecordstucompany where mrsc_id = $a and approve_status ='1'";
          ...
          <div class="form-group label-floating has-feedback">
            ...
            <textarea class="form-control css-require" rows="4" placeholder="กรอกความคิดเห็นเพิ่มเติม" name="sccompany_detail"></textarea>
          </div>
          <input type="hidden" name="Sc" value="<?echo $student_code?>">
          <input type="hidden" name="Mc" value="<?echo $row[3]?>">
          <input type="hidden" name="Cc" value="<?echo $row[2]?>">
