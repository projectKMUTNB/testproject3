...
<body>
      ...
      <div class="modal-body">
        <div class="box-body table-responsive no-padding">
            <form role="form" id="form" name="form" method="POST" action="teacherformqcompanyQuery.php">
              <table class="table table-hover">
               <tbody>
                ...
                <?php
                $i=0;
                $query = "SELECT * FROM mainrecordcompany where approve_status = '1'";
                $query = mysqli_query($conn,$query);
                $i=1;
                while ($m=mysqli_fetch_array($query))
                {
                  $a++;
                  $query1 = "SELECT crc_name FROM choicerecordcompany where mrc_id = $a and approve_status = '1'";
          ...
          <div class="form-group label-floating has-feedback">
            ...
            <textarea class="form-control css-require" rows="4" placeholder="กรอกความคิดเห็นเพิ่มเติม" name="commentcom_detail"></textarea>
          </div>
          <input type="hidden" name="Sc" value="<?echo $student_code?>">
          <input type="hidden" name="Tc" value="<?echo $_SESSION['teacher_code']?>">
